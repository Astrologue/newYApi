    ...
    <Modal visible={visible} title={title} onCancel={onCancel} onOk={onCreate}>
      <Form layout="vertical">
        <FormItem label="集合名">
          ...
          })(<Input disabled={type === 'add-sub' ? true : false} />)}
        </FormItem>
        {type === 'add-sub' ? <FormItem label="子集合名">
          {getFieldDecorator('colSubName', {
            rules: [{ required: true, message: '请输入子集合命名！' }]
          })(<Input />)}
        </FormItem> : ''}
...
export default class InterfaceColMenu extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.interfaceColList !== nextProps.interfaceColList) {
      this.setState({
        list: this.switchList(nextProps.interfaceColList)
      });
    }
  }
  ...
  async getList() {
    ...
    this.setState({
      list: this.switchList(r.payload.data.data)
    });
    return r;
  }

  addorEditCol = async () => {
    const { colName: name, colSubName: colSubName, colDesc: desc } = this.form.getFieldsValue();
    const { colModalType, editColId: col_id } = this.state;
    const project_id = this.props.match.params.id;
    let res = {};
    if (colModalType === 'add' || colModalType === 'add-sub') {
      const parmas = { name, desc, project_id };
      if (colModalType === 'add-sub') {
        parmas.parent_id = col_id;
        parmas.name = colSubName;
      } else {
        parmas.parent_id = 0;
      }
      res = await axios.post('/api/col/add_col', parmas);
  ...
  copyInterfaceCol = async (item, _parentId) => {
    const { desc, project_id, _id: col_id } = item;
    console.log(item);
    let { name, parent_id } = item;
    name = `${name} copy`;
    ...
    // 添加集合
    const add_col_res = await axios.post('/api/col/add_col', { name, desc, parent_id, project_id });
  ...
  // 递归克隆
  copyChildrenDeep = (item, parentId) => {
    if (!item || !item.length) {
      return;
    }
    item.map(childItem => {
      this.copyInterfaceCol(childItem, parentId).then(val => {
        this.getList();
        if (childItem.children && childItem.children.length) {
          this.copyChildrenDeep(childItem.children, val);
        }
      });
    })
  }
  ...
  copyInterface = async item => {
    ...
    this.copyInterfaceCol(item).then(val => {
      if (item.children && item.children.length) {
        this.copyChildrenDeep(item.children, val);
        this.getList();
    ...
    // const { desc, parent_id, project_id, _id: col_id } = item;
    // let { name } = item;
    // name = `${name} copy`;

    // // const newList = this.findColData(this.state.list,_id);
  ...
  showColModal = (type, col) => {
    let editCol = { colName: '', colDesc: '' };
    if (type === 'edit') {
      editCol = { colName: col.name, colDesc: col.desc };
    } else if (type === 'add-sub') {
      editCol = { colName: col.name, colSubName: '', colDesc: '' };
    }
    this.setState({
  ...
  filterCol = e => {
    ...
    // console.log('newList',newList);
    const list = this.switchList(this.props.interfaceColList)
    this.setState({
      filterValue: value,
  ...
  filterList = list => {
    ...
    function nodefilter(node) {
      ...
      let _filter = that.state.filterValue
      if (node.name.indexOf(_filter) === -1) {
        node.caseList = node.caseList.filter(inter => {
          if (
            inter.casename.indexOf(_filter) === -1 &&
            inter.path.indexOf(_filter) === -1
          ) {
            return false;
          }
          arr.indexOf('col_' + node._id) === -1 ? arr.push('col_' + node._id) : '';
    ...
    const renderItemLoop = list => {
      return list.map(col => {
        return (
              ...
              <div className="menu-title">
                ...
                <div className="btns">
                  <Tooltip title="删除集合">
                    <Icon
                      type="delete"
                      style={{ display: list.length > 1 || col.parent_id !== 0 ? '' : 'none' }}
                      className="interface-delete-icon"
                      onClick={() => {
                        this.showDelColConfirm(col._id);
                      }}
          ...
          >
            {col.children && col.children.length && renderItemLoop(col.children)}
