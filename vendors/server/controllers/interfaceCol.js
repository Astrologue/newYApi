...
class interfaceColController extends baseController {
      ...
      this.colModel.get(params.col_id).then(col => {
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 在接口集 <a href="/project/${params.project_id
            }/interface/col/${params.col_id}">${col.name}</a> 下添加了测试用例 <a href="/project/${params.project_id
    ...
    try {
      ...
      for (let i = 0; i < params.interface_list.length; i++) {
        ...
        this.colModel.get(params.col_id).then(col => {
          yapi.commons.saveLog({
            content: `<a href="/user/profile/${this.getUid()}">${username}</a> 在接口集 <a href="/project/${params.project_id
              }/interface/col/${params.col_id}">${col.name}</a> 下导入了测试用例 <a href="/project/${params.project_id
      ...
      this.colModel.get(caseData.col_id).then(col => {
        yapi.commons.saveLog({
          content: `<a href="/user/profile/${this.getUid()}">${username}</a> 在接口集 <a href="/project/${caseData.project_id
            }/interface/col/${caseData.col_id}">${col.name}</a> 更新了测试用例 <a href="/project/${caseData.project_id
   ...
   * @returns 
   */
  async recursionDelCol(id) {
    //删除子集
    let children = await this.colModel.getChildByid(id);
    if (children) {
      for (const category of children) {
        await this.recursionDelCol(category._id)
      }
    }
    /*      children && children.forEach(async category => {
         await this.recursionDelCol(category._id)
      }) */
    let interfaceData = await yapi.commons.getCaseList(id);
    if (interfaceData && interfaceData.data && interfaceData.data.length) {
      interfaceData.data.forEach(async item => {
        try {
          yapi.emitHook('interface_del', item._id).then();
          await this.caseModel.delByCol(item._id);
        } catch (e) {
          yapi.commons.log(e.message, 'error');
        }
      });
    }
    await this.colModel.del(id);
    return await this.caseModel.delByCol(id);
  }
  ...
  async delCol(ctx) {
    try {
      ...
      // 递归删除
      let result = await this.recursionDelCol(id);
